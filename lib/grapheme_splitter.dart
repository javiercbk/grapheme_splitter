...
bool isSurrogate(String str, int pos) {
  return  0xd800 <= str.codeUnitAt(pos) && str.codeUnitAt(pos) <= 0xdbff && 
      0xdc00 <= str.codeUnitAt(pos + 1) && str.codeUnitAt(pos + 1) <= 0xdfff;
}
...
int shouldBreak(int start, List<int> mid, int end) {
  ...
  if(previousNonExtendIndex != -1 && [E_Base, E_Base_GAZ].indexOf(all[previousNonExtendIndex]) != -1 &&
    all.length > previousNonExtendIndex + 1 && sliceFromEnd(all, previousNonExtendIndex + 1).every((c) => c == Extend) &&
    next == E_Modifier){
    return NotBreak;
  }
